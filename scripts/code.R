...
models <- function(data.models, variable){
  ...
  if(variable== "rgr"){
    m0 <- lm(log(rgr) ~1, data = data.models)
...
pca.vis <- function(data,pca,axis1,axis2,name){
  autoplot(pca, 
           label= F, # if T, size= 0
           data=data, #dados originais inseridos na pca
           colour="log.gr.dbh", #coluna que indica os grupos de cores
           label.size=4, #tamanho dos rotulos
           shape="Forest.stratum", #omite o simbolo
           size="growth_dbh",
           loadings=TRUE, #inclui as setas com as variaveis
           loadings.label.repel = TRUE,
           loadings.colour="black", #edita a cor das setas
           loadings.label=TRUE, #mostra o rotulo das variaveis
           loadings.label.size=4, #altera o tamanho do rotulo das variaveis
           loadings.label.colour="black", #altera cor do rotulo das variaveis
           x=axis1, y=axis2) + #PC to be plotted
    my_theme+
    theme(aspect.ratio=1)+
    theme(text = element_text(size=15))+
    scale_colour_gradient(low = "#F0E442",high = "#009E73")+
...
total_best.model <- total$m7 #parcimonia 
...
unders_best.model <- unders$m2 #parcimonia
...
#write.table(results, "results/results.txt")
#write.table(model.sel, "results/model.selection.txt")
...
#png('results/PC1 a PC3.png', units="in", width=13, height=8, res=300)
gridExtra:: grid.arrange(a,e,c,b,f,d, ncol=3)
#dev.off()